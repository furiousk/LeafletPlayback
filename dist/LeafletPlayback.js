  ...
  statics: {
    ...
    DateStr: function(time){
		...
		if( time < 1 ){
			...
			return new Date(time).toLocaleDateString('pt-BR',options);
		}
    },
...
L.Playback.MoveableMarker = L.Marker.extend({
	...
	arrayContainsArray: function( superset, subset ){
		...
		var searchJson = JSON.stringify( subset );
		var arrJson    = superset.map( JSON.stringify );
    ...
    move: function (latLng, transitionTime) {
		...
		var key = this.arrayContainsArray( this.feature.geometry.coordinates, [ latLng.lng, latLng.lat ] );
        ...
        if( this._popup ){
			if( !this.feature.properties.speed[ key ] ){}else{
				var speed = 'velocidade: ' + this.feature.properties.speed[ key ] + 'km/h';
				this._popup.setContent( this.getPopupContent() + speed );
			}
        }
    },
